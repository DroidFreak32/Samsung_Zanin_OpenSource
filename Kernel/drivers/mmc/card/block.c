	...
	struct mmc_blk_request *brq)
{
	...
	if (err){
		pr_err("%s: We couldn't get a response from the card.  Give up\n",
		       req->rq_disk->disk_name);
		return ERR_ABORT;
	}
	...
	if (R1_CURRENT_STATE(status) == R1_STATE_DATA ||
		...
		printk("%s-sends stop_cmd(CMD12) status:data(read) or rcv(write) - stop_status:0x%x\n",__func__,stop_status);
		if (err)
...
static int mmc_blk_issue_rw_rq(struct mmc_queue *mq, struct request *req)
{
	...
	do {
		...
		if (brq.sbc.error || brq.cmd.error || brq.stop.error) {
			...
			case ERR_RETRY:
				if (retry++ < 5){
					pr_err("%s %s: retry cnt:%d\n",__func__,req->rq_disk->disk_name,retry);
					pr_err("%s %s: brq.sbc.error:%d retry:%d brq.cmd.error:%d %d CMD%d, brq.stop.error:%d %d\n"
							,__func__,req->rq_disk->disk_name
							,brq.sbc.error,brq.sbc.retries
							,brq.cmd.error,brq.cmd.retries,brq.cmd.opcode
							,brq.stop.error,brq.stop.retries);
