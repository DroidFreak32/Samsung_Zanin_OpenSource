...
static void sdhci_dump_cmd(struct sdhci_host *host)
{
	struct mmc_command *cmd = host->cmd;
	struct mmc_data *data = host->data;
	struct mmc_command	*sbc = host->mrq->sbc;

	if(cmd){
		printk(KERN_DEBUG DRIVER_NAME "CMD%d Arg:0x%x, Flag:0x%x, Err:%d\n"
			,cmd->opcode,cmd->arg,cmd->flags,cmd->error);	
	}

	if(data){
		printk(KERN_DEBUG DRIVER_NAME "Blk_Size:%d NumOfBlk:%d Err:%d Flags:0x%x\n"
			,data->blksz,data->blocks, data->error, data->flags);			
	}

	if(sbc){
		printk(KERN_DEBUG DRIVER_NAME "SET_BLK_COUNT CMD%d Arg(BLK_CNT):0x%x, Flag:0x%x, Err:%d\n"
			,cmd->opcode,cmd->arg,cmd->flags,cmd->error);				
	}
