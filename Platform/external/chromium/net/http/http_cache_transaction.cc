...
int HttpCache::Transaction::DoNetworkRead() {
  ...
  // SAMSUNG CHANGE : check null pointer access
  if (network_trans_ == NULL) {
    LOG(ERROR) << "network_trans_ is null!";
    return ERR_UNEXPECTED;
  }
...
// SAMSUNG CHANGE: checking if entry_ existing.
  if (!entry_) {
...
// SAMSUNG CHANGE: checking if disk_entry existing.
...
int HttpCache::Transaction::WriteResponseInfoToEntry(bool truncated) {
  ...
  // SAMSUNG CHANGE : check null pointer >>
  if (!entry_->disk_entry) {
    LOG(WARNING) << "entry_->disk_entry is null";
    return OK;
  }
