  ...
  if (doomed_) {
    ...
    for (int index = 0; index < kNumStreams; index++) {
      if (user_buffers_[index].get()) {
        if (!Flush(index, 0)){	//SAMSUNG CHANGE: If flush failure of any index happened, the entry has to be checked as dirty bit.
          ret = false;			//SAMSUNG CHANGE: If flush failure of any index happened, the entry has to be checked as dirty bit.
